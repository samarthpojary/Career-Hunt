...
$candresume=md5($resume).time().$extension;
move_uploaded_file($_FILES["resume"]["tmp_name"],"images/".$candresume);
$message='';
$remark='';
$status='';

$query=mysqli_query($con,"insert into tblapplyjob(UserId,JobId,Resume,Message,Remark,Status) values('$userid','$jobid','$candresume','$message','$remark','$status')");
if($query)
{
...
<?php } else {?>
  ...
  <tr>
    <td colspan="2" style="text-align:center"><button type="submit" name="submit" class="btn btn-primary">Apply Now</button></td>
  </tr>
<?php } ?>
  </form>
  </table>
